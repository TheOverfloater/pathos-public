...
bool ALD_Load( daystage_t stage, byte** pdestarrays )
{
	...
	// This is a temp file only used in case we want to restore
	if(stage == DAYSTAGE_NORMAL_RESTORE)
		filename << "_restore";
	...
	case DAYSTAGE_DAYLIGHT_RETURN:
		{
			if(worldLightmapLayerCount == NB_SURF_LIGHTMAP_LAYERS)
		...
		case ALD_COMPRESSION_MINIZ:
			{
				...
				if(ploadheader->lightdatasize != destinationsize)
				{
...
void ALD_ExportLightmaps( aldcompression_t compressionType, daystage_t daystage )
{
	...
	case DAYSTAGE_DAYLIGHT_RETURN:
		{
			if(layercount == NB_SURF_LIGHTMAP_LAYERS)
	...
	const byte* poriginal = FL_LoadFile(filepath.c_str());
	if(poriginal)
	{
		...
		if(poldhdr->lightdatasize == pworldmodel->lightdatasize)
		{
...
{
	// If an ALD file exists, write a temporary restore file
	if(ALD_FileExists())
