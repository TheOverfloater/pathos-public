...
brushmodel_t* PBSPV3_Load( const byte* pfile, const dpbspv3header_t* pheader, const Char* pstrFilename )
{
	...
	// Load the lumps
	if(!PBSPV3_LoadVertexes(pfile, (*pmodel), pheader->lumps[PBSPV3_LUMP_VERTEXES])
		...
		|| !PBSPV3_LoadVertexLighting(pfile, (*pmodel), pheader->lumps[PBSPV3_LUMP_VERTEX_LIGHTING], SURF_LIGHTMAP_DEFAULT)
		|| !PBSPV3_LoadVertexLighting(pfile, (*pmodel), pheader->lumps[PBSPV3_LUMP_VERTEX_LIGHTING_AMBIENT], SURF_LIGHTMAP_AMBIENT)
		|| !PBSPV3_LoadVertexLighting(pfile, (*pmodel), pheader->lumps[PBSPV3_LUMP_VERTEX_LIGHTING_DIFFUSE], SURF_LIGHTMAP_DIFFUSE)
		|| !PBSPV3_LoadVertexLighting(pfile, (*pmodel), pheader->lumps[PBSPV3_LUMP_VERTEX_LIGHTING_VECTORS], SURF_LIGHTMAP_VECTORS)
...
//
//=============================================
bool PBSPV3_DecompressLightingData( const byte* pfile, brushmodel_t& model, const dpbspv3lump_t& lump, color24_t*& pdestptr, Uint32& destsize, byte*& poriginaldataptr, Uint32& originalsize, Int32& compression, Int32 compressionlevel )
{
	const dpbspv3lmapdata_t* plightmapdata = reinterpret_cast<const dpbspv3lmapdata_t*>(pfile + lump.offset);
...
bool PBSPV3_LoadDefaultLighting( const byte* pfile, brushmodel_t& model, const dpbspv3lump_t& lump )
{
	...
	// Check if sizes are correct
	if(lump.size != sizeof(dpbspv3lmapdata_t))
	{
		Con_EPrintf("%s - Inconsistent lump size in '%s'.\n", __FUNCTION__, model.name.c_str());
		return false;
	}

	return PBSPV3_DecompressLightingData(pfile, model, lump, model.plightdata[SURF_LIGHTMAP_DEFAULT], model.lightdatasize, 
...
bool PBSPV3_LoadLightingDataLayer( const byte* pfile, brushmodel_t& model, const dpbspv3lump_t& lump, surf_lmap_layers_t layer )
{
	...
	Uint32 datasize = 0;
	bool result = PBSPV3_DecompressLightingData(pfile, model, lump, model.plightdata[layer], datasize, 
...
//
//=============================================
bool PBSPV3_LoadVertexLighting(const byte* pfile, brushmodel_t& model, const dpbspv3lump_t& lump, surf_lmap_layers_t layer)
{
	if (!lump.size)
		return true;
	...
	Uint32 dummy_compressed_size;
	byte* dummy_orig_ptr = nullptr;
	Int32 dummy_comp, dummy_lev;

	bool result = PBSPV3_DecompressLightingData(pfile, model, lump, model.pvertexlightdata[layer], decompressed_size,
		dummy_orig_ptr, dummy_compressed_size, dummy_comp, dummy_lev);

	if (result)
	{
		if (layer == SURF_LIGHTMAP_DEFAULT)
		{
			model.vertexlightdatasize = decompressed_size;
		}
		else
		{
			if (decompressed_size != model.vertexlightdatasize)
			{
				Con_EPrintf("%s - Inconsistent vertex light size for layer %d (Got %d, expected %d).\n", __FUNCTION__, layer, decompressed_size, model.vertexlightdatasize);
			}
		}
	}

	Con_Printf("Loaded %d bytes of vertex lighting for layer %d\n", decompressed_size, layer);
	return result;
}