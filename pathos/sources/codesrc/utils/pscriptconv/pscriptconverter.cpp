...
// Object declaration
CParticleScriptConverter gScriptConverter;

//===============================================
// @brief Constructor for CParticleScriptConverter class
//
//===============================================
CParticleScriptConverter::CParticleScriptConverter( void )
{
}

//===============================================
// @brief Destructor for CParticleScriptConverter class
//
//===============================================
CParticleScriptConverter::~CParticleScriptConverter( void )
{
...
//===============================================
// @brief Processes a cluster particle script
//
// @param pstrScriptFile Path to script file
// @param pData File data
// @param pstrOutputPath Output path
// @return TRUE if successful, FALSE otherwise
//===============================================
bool CParticleScriptConverter::ProcessClusterScript( const Char* pstrScriptFile, const Char* pData, const Char* pstrOutputPath )
{
	CString line;
	CString f;
	...
	const byte* pstrdata = reinterpret_cast<const byte*>(f.c_str());
	if(!g_fileInterface.pfnWriteFile(pstrdata, f.length(), finalPath.c_str(), false))
	{
...
//===============================================
// @brief Processes a particle script
//
// @param pstrScriptFile Path to script file
// @param pData File data
// @param pstrOutputPath Output path
// @return TRUE if successful, FALSE otherwise
//===============================================
bool CParticleScriptConverter::ProcessParticleScript( const Char* pstrScriptFile, const Char* pData, const Char* pstrOutputPath )
{
	...
	while(1)
	{
		...
		{
			WarningMsg("Warning! Unknown field: %s in %s\n", field, pstrScriptFile);
		}
	}
...
//
// @param pstrPath Path to script file
// @param pstrOutputPath Output path
// @return TRUE if successful, FALSE otherwise
//===============================================
bool CParticleScriptConverter::ProcessScript( const Char* pstrPath, const Char* pstrOutputPath )
{
	Msg("Processing script '%s'.\n", pstrPath);

	const byte* pfile = g_fileInterface.pfnLoadFile(pstrPath, nullptr);
	if(!pfile)
	{
	...
	const Char* pstringdata = reinterpret_cast<const Char*>(pfile);
	CString line;
	Common::ReadLine(pstringdata, line);

	bool result;
	if(qstrstr(line.c_str(), ".txt") == nullptr)
		result = ProcessParticleScript(pstrPath, pstringdata, pstrOutputPath);
	else
		result = ProcessClusterScript(pstrPath, pstringdata, pstrOutputPath);
