...
static LRESULT CALLBACK EditWndProc (HWND hwnd, UINT uMessage, WPARAM wParam, LPARAM lParam)
{
	...
	if ((uMessage == WM_CHAR) && (wParam == VK_RETURN))
	{
		...
		// someone responds
		mxLineEdit *lineEdit = (mxLineEdit *) GetWindowLongPtr (hwnd, GWL_USERDATA);
		mxEvent event;
...
mxLineEdit::mxLineEdit (mxWindow *parent, int x, int y, int w, int h, const char *label, __int64 id, int style)
