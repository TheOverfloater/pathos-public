...
static surfchain_t* ReadSurfs(FILE* file)
{
    ...
    while (1)
    {
        ...
        {
#ifdef HLCSG_HLBSP_DOUBLEPLANE
			Developer (DEVELOPER_LEVEL_MEGASPAM, "inaccuracy: average %.8f qmax %.8f\n", inaccuracy_total / inaccuracy_count, inaccuracy_max);
#endif
            break;
        }
#ifdef ZHLT_DETAILBRUSH
...
static void     Settings()
{
    ...
    Log("qmax node size       [ %7d ] [ %7d ] (Min %d) (Max %d)\n",
    ...
    for (i = 1; i < argc; i++)
    {
        ...
        else if (!strcasecmp(argv[i], "-maxnodesize"))
        {
            if (i + 1 < argc)	//added "1" .--vluzacn
            {
                ...
                {
                    Warning
                        ("Maximum value for qmax node size is %i, '-maxnodesize %i' ignored",
                ...
                {
                    Warning
                        ("Mininimum value for qmax node size is %i, '-maxnodesize %i' ignored",
