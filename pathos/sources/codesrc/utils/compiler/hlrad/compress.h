	...
	(vector_type t, void *s, const float *f1, const float *f2, const float *f3)
{
	...
	case VECTOR24:
		...
		{
			qmax = *p1>*p2? (*p1>*p3? *p1: *p3): (*p2>*p3? *p2: *p3);
			qmax = float_istoobig (qmax)? 0x7F : float_istoosmall (qmax)? 0x60 : bitget (qmax, 23, 31);
			i1 = float_istoobig (*p1)? ~0u : (bitget (*p1, 0, 23) | bitput (1, 23, 24)) >> (1 + qmax - bitget (*p1, 23, 31));
			i2 = float_istoobig (*p2)? ~0u : (bitget (*p2, 0, 23) | bitput (1, 23, 24)) >> (1 + qmax - bitget (*p2, 23, 31));
			i3 = float_istoobig (*p3)? ~0u : (bitget (*p3, 0, 23) | bitput (1, 23, 24)) >> (1 + qmax - bitget (*p3, 23, 31));
		}
		if (t == VECTOR32)
			m[0] = 0
