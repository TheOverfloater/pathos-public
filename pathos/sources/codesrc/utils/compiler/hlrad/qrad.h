...
#define DEFAULT_SMOOTHING2_VALUE	-1
#endif
...
enum ald_datatype_t
{
	ALD_DATA_NIGHTSTAGE = 0,
	ALD_DATA_DAYLIGHT_RETURN
};

extern bool ExportALDData( ald_datatype_t type );
...
#define LIGHTMAPTOTEXTURE_GAMMA 1.0 // only approximate. Because the actual curve by which the engine convert lightmap data to screen brightness is very complicated.
#endif
...
#endif
#ifdef HLRAD_ADJUSTTEXTURE
	extern vec3_t *g_texturebrightness; //[nummiptex]
#endif
#ifdef HLRAD_AUTOCORING
...
__forceinline int TestPointOpaque (int modelnum, const vec3_t modelorigin, bool solid, const vec3_t point) // "inline" does nothing
{
...
extern void		snap_to_winding_noedge(const Winding& w, const dplane_t& plane, vec_t* point, vec_t width);
#endif
