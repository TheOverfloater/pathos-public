...
unsigned int	g_dmarksurfaces[MAX_MAP_MARKSURFACES];
...
static void     SwapBSPFile(const bool todisk)
{
    ...
    {
        g_dfaces[i].texinfo = LittleLong(g_dfaces[i].texinfo);
        g_dfaces[i].planenum = LittleLong(g_dfaces[i].planenum);
        g_dfaces[i].side = LittleLong(g_dfaces[i].side);
        g_dfaces[i].lightofs = LittleLong(g_dfaces[i].lightofs);
        g_dfaces[i].firstedge = LittleLong(g_dfaces[i].firstedge);
        g_dfaces[i].numedges = LittleLong(g_dfaces[i].numedges);
    }
    ...
    for (i = 0; i < g_numnodes; i++)
    {
        ...
        g_dnodes[i].firstface = LittleLong(g_dnodes[i].firstface);
        g_dnodes[i].numfaces = LittleLong(g_dnodes[i].numfaces);
    }
    ...
    for (i = 0; i < g_numleafs; i++)
    {
        ...
        g_dleafs[i].nummarksurfaces = LittleLong(g_dleafs[i].nummarksurfaces);
    ...
    {
        g_dmarksurfaces[i] = LittleLong(g_dmarksurfaces[i]);
    }
...
int CountBlocks ()
{
	...
	for (k = 0; k < g_numfaces; k++)
	{
		...
		if (extents[0] < 0 || extents[1] < 0 || extents[0] > qmax (512, MAX_SURFACE_EXTENT * TEXTURE_STEP) || extents[1] > qmax (512, MAX_SURFACE_EXTENT * TEXTURE_STEP))
			// the default restriction from the engine is 512, but place 'qmax (512, MAX_SURFACE_EXTENT * TEXTURE_STEP)' here in case someone raise the limit
		{
...
#endif
extern bool g_stripexternals;
void            UnparseEntities( bool stripedd )
{
    char*           buf;
    char*           end;
    epair_t*        ep;
    ...
    *end = 0;
    ...
    for (i = 0; i < g_numentities; i++)
    {
		...
		{
			char szclassname[32];
			strcpy_s(szclassname, ValueForKey(&g_entities[i], "classname"));

			if (!strcmp(szclassname, "func_detail_ext"))
				continue;
		}
