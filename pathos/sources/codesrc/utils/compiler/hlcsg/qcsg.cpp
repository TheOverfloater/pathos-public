...
static void     Settings()
{
    ...
    Log("qmax texture memory    [ %7d ] [ %7d ]\n", g_max_map_miptex, DEFAULT_MAX_MAP_MIPTEX);
	Log("qmax lighting memory   [ %7d ] [ %7d ]\n", g_max_map_lightdata, DEFAULT_MAX_MAP_LIGHTDATA);
    ...
    Log("strip edd             [ %7s ] [ %7s ]\n", g_stripexternals  ? "on" : "off", false      ? "on" : "off");
...
int             main(const int argc, char** argv)
{
    ...
    for (i = 1; i < argc; i++)
    {
		...
		else if (!strcasecmp(argv[i], "-stripedd"))
		{
			g_stripexternals = true;
		}
	...
	if(g_stripexternals)
	{
		for (i = 0; i < g_numentities; i++)
		{
			...
			if (!strcmp (ValueForKey (ent, "classname"), "func_detail_ext"))
			{
				Log("Entity %s marked for removal\n", ValueForKey (ent, "classname"));
				SetKeyValue(ent, "remove", "1");
			}
		}
	}
